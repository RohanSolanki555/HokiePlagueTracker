...
The dashboard map initially shows the last seven days of reports within 3 km of the [Virginia Tech Drillfield](https://www.coordinatesfinder.com/coordinates/13540-virginia-tech-drill-field-blacksburg-va) (37.2274294, -80.4222303). Pan and zoom the map, then select **Center on Drillfield** to return to campus. The mapped area stays centered on the Drillfield. Controls below the map filter by **30 days**, **14 days**, or **7 days**, and change the circle radius. They update immediately; the period also applies to summary and dorm statistics, while the radius only affects the map. Pins show report counts. Selecting a pin or a location in the list shows its counts, average severity, and change from the previous period.

The **Illness reports in Blacksburg** card between the top statistics and the map shows each reported illness, its count, and its share of all submitted reports. It shares its data with all four top statistic cards, including dorm, off-campus, and older reports across the tracker, independently of the map radius. Both summaries follow the selected report period and refresh together when reports are submitted or dashboard data is refreshed. These are self-reported submissions, not a count of unique residents or confirmed cases.
...
The existing `/api/stats/summary` returns actual database totals across all locations, with `weekly_change` nullable when there is no prior baseline. The dashboard's four top statistic cards and community illness breakdown use a single `/api/stats/illnesses` response so their totals agree. The map endpoint independently filters pins and anonymous home areas to the selected radius. Counts refresh immediately after a successful submission, every 15 seconds while the page is visible, and when the user returns to the tab. Counts are calculated from report rows, so failed submissions never increment them.

`GET /api/stats/illnesses?days=7` powers both dashboard summaries. It accepts `days=all` or an integer from 1 through 30 (default 7), returning `days`, `generated_at`, `total_reports`, a `stats` object with the same fields shown above, and `illnesses` entries with `illness` and `reports`. It counts all reports in the selected rolling period, even reports without map coordinates, and excludes future timestamps. `stats.total_reports`, `total_reports`, and the sum of the illness counts are equal. Results are ordered by count descending, then illness name. Flu, Flu A, and Flu B remain separate; missing illness values appear as **Not specified**. Invalid periods return 400 and database failures return 503. It uses the existing reports columns and requires no additional SQL migration.
